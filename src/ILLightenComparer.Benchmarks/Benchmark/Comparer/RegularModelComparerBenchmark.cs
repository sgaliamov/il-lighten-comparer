...
namespace ILLightenComparer.Benchmarks.Benchmark.Comparer
{
    public class RegularModelComparerBenchmark : ComparersBenchmark<MovieModel>
    {
        ...
        private static readonly IComparer<MovieModel> ILLightenComparer = new ComparerBuilder(c => c
            .SetDefaultCyclesDetection(false)
            .SetDefaultFieldsInclusion(false))
            .For<MovieModel>()
            .GetComparer();

        private static readonly IComparer<MovieModel> NitoComparer = Nito.Comparers.ComparerBuilder
            .For<MovieModel>()
            .OrderBy(x => x.Actors)
            .ThenBy(x => x.Genre)
            .ThenBy(x => x.Id)
